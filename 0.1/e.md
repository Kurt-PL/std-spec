...
This annex is normative with respect to the revision of this specification. It governs the drafting of the body in this and in future editions. A violation of a rule of this annex is an editorial defect, not a technical one.

## E.1 Principles

This specification is both a specification and a standard. The two words name different properties of the same document, and each property imposes its own discipline on the way the document is written.
...
A standard is a **rule**. A rule that must be defended before it can be followed is not a rule but a proposal. Every sentence in this document that defends, softens, or qualifies a rule in response to an anticipated objection is treating the rule as a proposal.

A standard is **official** — it is the one authority in its domain. A standard that defers to another authority for part of its content has ceded that part of its domain. Where this specification depends on an external standard for a precise definition, that dependency is declared, bounded, and isolated.

A standard is **a unit** — singular and consistent. The same concept has one name. The same kind of rule appears in the same form. Variation for stylistic purposes is ambiguity.

A standard is used **when producing**. It is a reference consulted at the point of use, not a narrative read from beginning to end. It must be self-contained at every point of entry. A reader who arrives at any clause must find everything needed to apply that clause.
...
A specification is a **description**. Every sentence in this document that is not a description of the language is a foreign body.

A specification describes how something **is**. It is not a record of how the language came to be, how it differs from what it was, or how it compares to what it is not. What the specification does not describe does not exist.

A specification is **detailed**. A description that permits two readings is incomplete. Precision is not a virtue here but a minimum condition of the description being a description at all.
...
— **Precision** — a sentence that permits two interpretations describes two languages. It is a defect.
...
The principles above are not independent axioms to be weighed against one another. They are consequences of what this document is.
...
Code appearing in code form within an Example block is not subject to these prohibitions. Comment text within the code, and the prose surrounding it, may name whatever the example needs named and may take whatever form the example requires. It shall not take a term’s meaning from outside this specification.

### Extraneous concept

A sentence imports an extraneous concept when it relies on a definition, model, or vocabulary that originates outside this specification.

**Borrowed definition.** A sentence that uses a technical term this specification has not defined is prohibited. Terms such as *calling convention*, *stack frame*, *byte*, *compilation*, *register* carry implicit implementation models not established by this specification. Where such a concept is required, it shall be derived and named from the Kurt machine. A term may coincide with a term used elsewhere after definition. The definition is the authority. Editors should seek the most technically precise term derivable from the Kurt machine rather than adopting an established name.
...
> *Values are stored in eight-bit bytes.* — prohibited twice over: *byte* carries an implicit model of storage not established here, and *eight-bit* contradicts `cellbits`, which this specification leaves to the execution environment. *Ex contradictione sequitur quodlibet*: the sentence renders the whole specification void.
...
> *A reference type does not name an object lifetime.* — prohibited for the reason construct negation is prohibited: the sentence imports a concept in order to deny it.
...
A sentence expresses self-doubt when it reacts to an anticipated reader response: a predicted misreading, a foreseen objection, or a presumed gap in understanding.
...
**Rhetorical repetition.** A sentence that restates a fact already stated in the same clause — at increased emphasis or altered phrasing — is prohibited.
...
**Defensive restatement.** A sentence that reproduces a rule defined in another clause, justified by local clarity, is prohibited.

> *This constraint is inherited from `numeric`; it is restated here for clarity.* — prohibited.

**Ritual qualification.** A sentence that accompanies a defined term with its defining conditions at each use site is prohibited. A defined term carries its full definition wherever it appears. Naming the term and, where a cross-reference is warranted, its defining clause is sufficient.
...
**Hedged assertion.** A normative sentence qualified with *generally*, *typically*, *usually*, *in most cases*, or equivalent is prohibited. A rule either holds or it has stated exceptions. Where exceptions exist, they shall be enumerated.
...
**Reader address.** A sentence that addresses a hypothetical reader is prohibited. The body of this specification states the rule and does not address questions about it.
...
> *The `airside`/`landside` distinction exists to make the boundary between safe and unsafe code visible at the call site.* — prohibited: the sentence states the purpose of the distinction, not the distinction.
...
**Design evaluation.** A sentence that asserts the merit, elegance, or superiority of a design choice is prohibited.
...
**Narrative connective.** A sentence that frames the specification as a progression is prohibited.
...
References to non-language technical standards — ISO/IEC/IEEE standards, IETF RFCs, and ECMA standards other than language standards — are permitted in normative text when this specification depends on an external standard for a precise definition that cannot be stated in self-contained terms. Such a reference shall name the standard and the specific version relied upon. Where a concept from an external standard can be defined in self-contained terms, the self-contained definition is preferred.

References to language standards are prohibited in body clauses. A language standard is any specification that defines the syntax and semantics of a programming language for the purpose of programme construction. The prohibition extends to indirect reference: naming a language-specific artefact — a calling convention named for a language, a representation described as originating in a language — constitutes a reference to that language.

## E.3 Notation

### Orthography

Each language edition follows the recognized standard orthography of its language. Where a language permits more than one standard, the edition designates one and applies it consistently.

#### English edition

The English edition uses British English, following Oxford spelling. The suffix derived from Greek is written *-ize* / *-ization*, not *-ise* / *-isation*: *organize*, *recognize*, *normalization*. A word not formed with this suffix retains *-ise* (*comprise*, *exercise*, *otherwise*); *-yse* is retained over *-yze* (*analyse*). *programme* is correct in all contexts, including a computer programme. Elsewhere the British form is used: *behaviour*, *colour*, *defence*, *licence* (noun).

Titles and headings, the title of this specification included, are written in sentence case. The first word begins with a capital letter unless it is set in monospaced type; any other word does so only where it is a proper noun.
...
Word choice and sentence construction take H. W. Fowler’s *A Dictionary of Modern English Usage* as their model.
...
Each technical term used in this specification shall have exactly one name and exactly one definition. Synonyms are prohibited: if two words could refer to the same concept, one shall be chosen and the other shall not appear. The chosen term shall be used consistently throughout.
...
A definition shall appear at the first point where the term is needed.

### Lists

A **prose enumeration** is used when the items are three or fewer, short, and integrate naturally into the grammatical structure of the sentence. A colon may introduce the list. Items are separated by commas.
...
The default markers and labelling schemes above may be varied to suit the rendering environment. Variation is permitted provided it is applied consistently within that environment, whatever the rendering cause of the variation.

An ordered list where no behavioural order exists is a false implication of sequencing and is an editorial defect. Conversely, a construct whose behavioural order is normatively significant shall not be presented as an unordered list or as prose enumeration.
...
Heading depth is relative to the page title. The page title is the top-level heading, whatever numbering depth it occupies. Each heading level below it stands one numbering level below the one above.

#### Examples (informative)

A page titled §7. Statements and control flow:
...
A page titled §7.2 Contract types:
...
A subclause carries the headings below that have applicable content, in the order given. A heading not listed here stands last.
...
8. Design rationale (informative) — motivation for the design choice.
9. Examples (informative) — illustrative code, free-form as to platform, external name, and commentary style, subject to the rule on borrowed definitions.
10. Notes (informative) — supplementary remarks.
11. Numbered sub-subclauses — last.

Where Notes restate the rules of their subclause in symbols, the symbols shall state those rules and no others. The prose is the rule. A difference between the two is an editorial defect, to be repaired in the symbols.

### Cross-references

A cross-reference should name the number of the clause or subclause it targets. A reference by description alone is permitted: *the clause on types*, *the subclause on reference casting*, *the rules defined above*.
